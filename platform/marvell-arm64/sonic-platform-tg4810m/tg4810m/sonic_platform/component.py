...
class Component(ComponentBase):
    ...
    def _get_cpld_version(self, cpld_number):
        ...
        if smbus_present == 0:
            cmdstatus, cpld_version = cmd.getstatusoutput('i2cget -y 2 0x41 0x2') 
        else:
            bus = smbus.SMBus(2)
            DEVICE_ADDRESS = 0x41
            DEVICE_REG = 0x2
    ...
    def get_firmware_version(self):
        ...
        if self.index == 1:
            cmdstatus, uboot_version = cmd.getstatusoutput('grep --null-data ver= /dev/mtd0ro |  cut -d= -f2- |  cut -c 1-26')
    ...
    def install_firmware(self, image_path):
        ...
        print(" install cpld {}".format(image_name) )
